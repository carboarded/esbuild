...
const tests = {
  serveTest({ testDir, esbuildPathWASM }) {
    try {
      child_process.execFileSync(
        "node",
        [esbuildPathWASM, "--servedir=.", "--log-level=warning"],
        {
          stdio: "pipe",
          cwd: testDir,
        }
      ...
      assert.strictEqual(
        err.stderr + "",
        `✘ [ERROR] The "serve" API is not supported when using WebAssembly\n\n`
      );
    }
  },
  ...
  basicStdinTest({ testDir, esbuildPathWASM }) {
    const stdout = child_process
      .execFileSync(
        "node",
        [esbuildPathWASM, "--format=cjs", "--log-level=warning"],
        {
  ...
  stdoutLargeTest({ testDir, esbuildPathNative, esbuildPathWASM }) {
    ...
    // Build with native
    const stdoutNative = child_process
      .execFileSync(esbuildPathNative, [entryPoint, "--log-level=warning"], {
        stdio: ["pipe", "pipe", "inherit"],
        cwd: testDir,
      })
      .toString();
    ...
    // Build with WASM
    const stdoutWASM = child_process
      .execFileSync(
        "node",
        [esbuildPathWASM, entryPoint, "--log-level=warning"],
        {
          stdio: ["pipe", "pipe", "inherit"],
          cwd: testDir,
        }
  ...
  outfileLargeTest({ testDir, esbuildPathNative, esbuildPathWASM }) {
    ...
    const stdoutNative = child_process
      .execFileSync(
        esbuildPathNative,
        [entryPoint, "--outfile=" + outfileNative, "--log-level=warning"],
        {
          stdio: ["pipe", "pipe", "inherit"],
          cwd: testDir,
        }
    ...
    child_process
      .execFileSync(
        ...
        ],
        {
          ...
          input:
            `export {default} from ` +
            JSON.stringify("./" + path.relative(testDir, packageJSON)),
        }
      )
      .toString();
...
async function main() {
  ...
  const esbuildPathNative = path.join(
    __dirname,
    "..",
  ...
  const esbuildPathWASM = path.join(
    __dirname,
    "..",
    "npm",
    "esbuild-wasm",
