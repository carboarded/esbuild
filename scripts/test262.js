...
async function main() {
  ...
  async function processFile(file) {
    ...
    if (yaml.flags) {
      if (yaml.flags.includes("onlyStrict"))
        content = '"use strict";\n' + content;
      if (yaml.flags.includes("module")) content = "export {};\n" + content;
    }

    if (
      skipList.includes(path.relative(test262Dir, file).replace(/\\/g, "/"))
    ) {
      return;
    }

    const result = await esbuildFile(content, {
      minify: false,
      sourcefile: file,
    });
