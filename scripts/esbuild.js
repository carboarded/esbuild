...
exports.buildWasmLib = async (esbuildPath) => {
  const npmWasmShimDirs = [path.join(repoDir, "npm", "esbuild-android-64")];
  ...
  for (const minify of [false, true]) {
    ...
    for (const [format, target] of Object.entries({
      umd: umdBrowserTarget,
      esm: esmBrowserTarget,
    })) {
    ...
    const browserCJS = childProcess
      .execFileSync(
        ...
        [
          ...
          "--define:WEB_WORKER_SOURCE_CODE=" +
            JSON.stringify(wasmWorkerCode.umd),
          "--banner:js=" + umdPrefix,
          "--footer:js=" + umdSuffix,
          "--log-level=warning",
        ].concat(minifyFlags),
      ...
      .toString()
      .replace("WEB_WORKER_FUNCTION", wasmWorkerCode.umd);
    fs.writeFileSync(
    ...
    const browserESM = childProcess
      .execFileSync(
        ...
        [
          ...
          "--define:ESBUILD_VERSION=" + JSON.stringify(version),
          "--define:WEB_WORKER_SOURCE_CODE=" +
            JSON.stringify(wasmWorkerCode.esm),
          "--log-level=warning",
        ].concat(minifyFlags),
        { cwd: repoDir }
      )
      .toString()
      .replace("WEB_WORKER_FUNCTION", wasmWorkerCode.esm);
    fs.writeFileSync(
